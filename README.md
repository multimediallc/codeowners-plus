...
`string_literals` ignores the wording of the strings which open a call to a translation function - `_`, `gettext`, `ngettext`, `t` or `trans` - and nothing else. A string is only recognized as copy when the call around it says what the string is for: a query, a path, a key and a sentence are all spelled the same way, so a string standing on its own is never ignored. Even inside such a call the string has to read as a sentence, and is left alone if it holds a placeholder, an address, or no space at all, since those spell something the reader cannot check by reading the sentence. Expect the payoff to be small: the flag only fires on a reworded sentence inside a translation call, so what it retains depends entirely on how much of a codebase is written that way, and in one which routes its copy any other way it will retain nothing at all. Enable it because that shape is common in your code, not in the expectation that it will retain much on its own.
